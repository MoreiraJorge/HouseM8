...
namespace HouseM8API.Data_Access
{
    ...
    public class WorkDAO : IWorkDAO
    {
        ...
        public Job Create(int id, Job model)
        {
            ...
            {
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = "INSERT INTO dbo.[PendingJobs] (JobId, MateId)" +
                    "VALUES (@jId, @mId); SELECT @@Identity";
        ...
        public bool MarkJobAsDone(int jobId, int userId)
        {
            ...
            if (foundMate)
            {
                ...
                {
                    cmd.CommandType = CommandType.Text;
                    cmd.CommandText = "DELETE FROM dbo.[PendingJobs] " +
                        "WHERE JobId = @jbId AND MateId = @mtId";

                    cmd.Parameters.Add("@jbId", SqlDbType.Int).Value = jobId;
                    cmd.Parameters.Add("@mtId", SqlDbType.Int).Value = userId;
