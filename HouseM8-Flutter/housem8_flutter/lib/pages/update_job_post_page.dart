...
class _UpdateJobPostPageState extends State<UpdateJobPostPage> {
  ...
  void updateDataFromService(JobPostPublication jobPostPublication) {
    ...
    if (jobPostPublication.address.streetNumber == null) {
      jobPostPublication.address.streetNumber = int.parse(
          RegExp(r"[0-9]\S+").allMatches(widget.post.address).elementAt(0).group(0));
    }

    if (jobPostPublication.address.postalCode == null) {
      jobPostPublication.address.postalCode =
          RegExp(r"[0-9]\S+").allMatches(widget.post.address).elementAt(1).group(0);
    }
    ...
    if (jobPostPublication.address.country == null) {
      jobPostPublication.address.country = widget.post.address.split(",").last.trim();
    }
